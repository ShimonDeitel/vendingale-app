import XCTest
@testable import Vendingale

@MainActor
final class VendingaleTests: XCTestCase {
    var store: VendingaleStore!

    override func setUp() async throws {
        store = VendingaleStore()
    }
    ...
    func testFreeLimitBlocksAddition() {
        store.isPro = false
        for i in 0..<(VendingaleStore.freeLimit + 5) {
            store.add(title: "Filler \(i)", amount: 1.0)
        }
        XCTAssertEqual(store.entries.count, VendingaleStore.freeLimit)
    }
    ...
    func testCanAddMoreWhenPro() {
        ...
        XCTAssertGreaterThan(store.entries.count, VendingaleStore.freeLimit)
    }
    ...
    func testFreeLimitAboveSeedCount() {
        XCTAssertGreaterThan(VendingaleStore.freeLimit, 3)
    }
}
